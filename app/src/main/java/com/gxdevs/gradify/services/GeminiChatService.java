...
public class GeminiChatService {
    ...
    private static final String GOOGLE_API_URL = "https://generativelanguage.googleapis.com/v1beta/models/" + MODEL_NAME
            + ":generateContent?key=";
    ...
    public void generateChatResponse(String API_KEY, List<Content> chatHistory, String currentVideoLink,
            String userPrompt, GeminiChatCallback callback) {

        JSONArray contentsArray = new JSONArray();
        JSONObject requestBodyJson = new JSONObject();

        try {
            // History processing: Assuming chatHistory from LectureActivity is pre-filtered
            // (e.g., max 3 user, 3 model) and ordered chronologically.
            if (chatHistory != null) {
    ...
    // Renamed and adapted for Google API structure if needed (current structure is
    // likely fine)
    ...
    @NonNull
    private JSONObject buildCurrentUserContentForGoogleApi(String currentVideoLink, String userPrompt)
